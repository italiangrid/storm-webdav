...
public class VOMSBeans {
  ...
  VOMSPreAuthDetailsSource vomsDetailsSource(VOMSACValidator validator,
      AuthorizationPolicyService ps, VOMapDetailServiceBuilder builder, @Value("${storm.nginx-reverse-proxy}") boolean nginxReverseProxy) {
    return new VOMSPreAuthDetailsSource(validator, ps, builder.build(), nginxReverseProxy);
  }

  @Bean
  VOMSAuthenticationFilter vomsAuthenticationFilter(VOMSPreAuthDetailsSource ds, @Value("${storm.nginx-reverse-proxy}") boolean nginxReverseProxy) {
